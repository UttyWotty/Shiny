...
TrainSet <- TrainSet[,-1] # take the first column out whixch is index
...
ui <- pageWithSidebar(
  ...
  sidebarPanel(
    ...
    sliderInput("Sepal.Length", label = "Sepal Length", value = 1.0,
                min = min(TrainSet$Sepal.Length),
                max = max(TrainSet$Sepal.Length)
    ),
    sliderInput("Sepal.Width", label = "Sepal Width", value = 1.6,
                min = min(TrainSet$Sepal.Width),
                max = max(TrainSet$Sepal.Width)),
    sliderInput("Petal.Length", label = "Petal Length", value = 1.0,
                min = min(TrainSet$Petal.Length),
                max = max(TrainSet$Petal.Length)),
    sliderInput("Petal.Width", label = "Petal Width", value = 0.1,
